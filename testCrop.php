...
// $imgtype = exif_imagetype($_FILES['imgFile']['tmp_name']);
$imginfo= getimagesize($_FILES['imgFile']['tmp_name']);
$imgtype = @end(explode("/",end($imginfo)));
...
// $fp = fopen('data.txt', 'w');
// fwrite($fp, 'hahha'.$exif['Orientation'].'hahhaha');
// fclose($fp);
switch($imgtype){
    case 'gif':
        $src_im = imagecreatefromgif($_FILES['imgFile']['tmp_name']);
    ...
    case 'jpeg':
        ...
        break;
    case 'png':
        $src_im = imagecreatefrompng($_FILES['imgFile']['tmp_name']);
        break;
}
if($crotate!=0){
    $src_im = imagerotate($src_im,$crotate,0);
}
...
// $src_im = imagescale ($src_im , $dwidth*$scale);
